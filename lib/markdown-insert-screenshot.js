...
export default {
  ...
  screenshotToRelativeDestination(e) {
    ...
    if (editor = atom.workspace.getActiveTextEditor()) {
      ...
      destination = path.join(editorPath.dir, basename).replace(/ /g, "\\ ")
      destinationPath = path.parse(destination)
      if ( !fs.existsSync(destinationPath.dir) ) {
        shelljs.mkdir('-p', destinationPath.dir)
      }
      this.executeShellCommand("screencapture -i -o " + destination)
    }
  }
};
